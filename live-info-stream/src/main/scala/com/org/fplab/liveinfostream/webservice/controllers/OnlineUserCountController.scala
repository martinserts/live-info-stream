...
class OnlineUserCountController[F[_]: Sync : Concurrent](stateRef: Ref[F, ApplicationState], topic: Topic[F, Option[String]]) {
  def addUser: F[ApiCommand] = updateStateAndPublishCommand(_ + 1)

  def removeUser: F[ApiCommand] = updateStateAndPublishCommand(_ - 1)

  private def updateState(updateUserCount: Int => Int): State[ApplicationState, ApiCommand] = for {
    _ <- State.modify((ApplicationState.webService composeLens WebServiceState.onlineUserCount).modify(updateUserCount))
    userCount <- State.inspect[ApplicationState, Int](_.webService.onlineUserCount)
